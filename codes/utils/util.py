...
def save_img_with_ratio(image_path, image, alignratio_path):
    align_ratio = (2 ** 16 - 1) / image.max()
    np.save(os.path.join(alignratio_path,), align_ratio)
    uint16_image_gt = np.round(image * align_ratio).astype(np.uint16)
...
def generate_paths(folder, name):
    id = name[:4]
    image_path = os.path.join(folder, "input", id+'.png')
    alignratio_path = os.path.join(folder, "restore", id+'_alignratio.npy')

    return image_path, alignratio_path

def generate_paths_withlabel(folder, label, name):
    id = name[:7]
    print(os.path.join(folder, "input", label))
    if not exists(os.path.join(folder, "input", label)):
        os.mkdir(os.path.join(folder, "input", label))
        os.mkdir(os.path.join(folder, "restore", label))
    image_path = os.path.join(folder, "input", label, id+'.png')
    alignratio_path = os.path.join(folder, "restore", label, id+'_alignratio.npy')
    return image_path, alignratio_path
