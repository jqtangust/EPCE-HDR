...
class LQGT_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # mu = [1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000, 200000, 500000, 1000000, 2000000]
        rdm = random.randint(0, 19)

        LQ_path = self.paths_LQ[index*20+rdm]
        ...
        # get GT alignratio
        filename = osp.basename(GT_path)[:4] + "_alignratio.npy"
...
class LQGT_dataset_Val(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.sizes_LQ, self.paths_LQ = util.get_image_paths(
            self.data_type, opt['dataroot_LQ'])
        self.sizes_GT, self.paths_GT = util.get_image_paths(
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            if GT_size != 0:
                rnd_h = random.randint(0, max(0, H - LQ_size))
                rnd_w = random.randint(0, max(0, W - LQ_size))
                img_LQ = img_LQ[rnd_h:rnd_h +
                                LQ_size, rnd_w:rnd_w + LQ_size, :]
                rnd_h_GT, rnd_w_GT = int(rnd_h * scale), int(rnd_w * scale)
        ...
        cond = torch.from_numpy(np.ascontiguousarray(
            np.transpose(cond, (2, 0, 1)))).float()
